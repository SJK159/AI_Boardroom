...
class GrowthAgent(SpecialistAgent):
    ...
    def analyze(self, query: str) -> list[Finding]:
        results = self._call_tools_parallel({
            "regional_sales_breakdown": (tools.regional_sales_breakdown, {"db": self.db}),
            "market_expansion_signals": (tools.market_expansion_signals, {"db": self.db}),
            "category_growth_rate": (tools.category_growth_rate, {"db": self.db}),
            "customer_acquisition_trend": (tools.customer_acquisition_trend, {"db": self.db}),
            "underperforming_region_diagnosis": (tools.underperforming_region_diagnosis, {"db": self.db}),
            "new_seller_onboarding_rate": (tools.new_seller_onboarding_rate, {"db": self.db}),
            "product_gap_analysis": (tools.product_gap_analysis, {"db": self.db}),
        })
        regional = results["regional_sales_breakdown"]
        expansion = results["market_expansion_signals"]
        cat_growth = results["category_growth_rate"]
        acquisition = results["customer_acquisition_trend"]
        underperforming = results["underperforming_region_diagnosis"]
        onboarding = results["new_seller_onboarding_rate"]
        gaps = results["product_gap_analysis"]
