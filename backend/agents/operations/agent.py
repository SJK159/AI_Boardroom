...
class OperationsAgent(SpecialistAgent):
    ...
    def analyze(self, query: str) -> list[Finding]:
        results = self._call_tools_parallel({
            "calculate_delivery_delay": (tools.calculate_delivery_delay, {"db": self.db}),
            "seller_performance_score": (tools.seller_performance_score, {"db": self.db}),
            "flag_late_shipments": (tools.flag_late_shipments, {"db": self.db}),
            "shipping_cost_analysis": (tools.shipping_cost_analysis, {"db": self.db}),
            "carrier_performance_comparison": (tools.carrier_performance_comparison, {"db": self.db}),
            "fulfillment_bottleneck_detection": (tools.fulfillment_bottleneck_detection, {"db": self.db}),
            "estimated_vs_actual_delivery_accuracy": (tools.estimated_vs_actual_delivery_accuracy, {"db": self.db}),
        })
        delay = results["calculate_delivery_delay"]
        seller_perf = results["seller_performance_score"]
        late_shipments = results["flag_late_shipments"]
        shipping_cost = results["shipping_cost_analysis"]
        carrier = results["carrier_performance_comparison"]
        bottleneck = results["fulfillment_bottleneck_detection"]
        accuracy = results["estimated_vs_actual_delivery_accuracy"]
